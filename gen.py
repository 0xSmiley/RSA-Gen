...
def genRSAkey(length):
    ...
    p=num_gen(length/2 +1)
    while(not is_probable_prime(p,64)):
        p+=2

    dif=length//2
    q=num_gen(length/2 +1)+dif      #Somamos dif a q para impedir uma ataque de Faulty key generation
    ...
    d=mulinv(e,f)                  #Calculo do inverso multiplicativo

    tuplo=(n,p,q,e,d)
    return tuplo

def num_gen(length):
    '''Bit mais significativo do lenght colocado a 1 de modo a garantir
